...
class AuthController extends Controller
{
    ...
    function getLogin(Request $request){
      ...
      if(Auth::check()) {
        ...
        return view('Admin.login', ['url' => $url]);
      }
    }
  ...
  function authenticate(Request $request){
    ...
    if (Auth::attempt(['email' => $data['username'], 'password' => $data['password'], 'active' => 1])) {
      $user = User::where('email', '=', $data['username'])->take(1)->get()->toArray();
      ...
      if(isset($url) && $url){
        ...
        return redirect()->intended(env('APP_URL').'admin?url='.$url);
      }
    } else {
      return redirect()->intended(env('APP_URL').'auth/login?url='.$url);
    }
  }
  ...
  function admin(Request $request){
    if(Auth::check()) {
      ...
      return redirect()->intended(env('APP_URL').'auth/login?url='.env('APP_HOST').$_SERVER['REQUEST_URI']);
    }
  }
