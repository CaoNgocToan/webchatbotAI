...
class LogController extends Controller
{
    ...
    function datatable(Request $request){
        ...
        if($logs){
           foreach($logs as $log){
            $link = '<a href="'.env('APP_URL').app()->getLocale().'/admin/logs/get-log/'.$log['_id'].'" data-toggle="modal" class="get_log" data-target="#logModal">'.$log['_id'] .'</a>';
    ...
    function get_log(Request $request, $locale='', $id =''){
        $log = Log::find($id);
        var_dump($log);
    }
}
