...
@section('css')
  <link href="{{ env('APP_URL') }}assets/admin/libs/select2/select2.min.css" rel="stylesheet" type="text/css" />
  <link rel="stylesheet" href="{{ env('APP_URL') }}assets/admin/libs/switchery/switchery.min.css">
  <link rel="stylesheet" href="{{ env('APP_URL') }}assets/admin/libs/magnific-popup/magnific-popup.css"/>
@endsection
@section('body')
<div class="row">
    <div class="col-12">
        <div class="card-box">
          <h3 class="m-t-0"><a href="{{ env('APP_URL') }}admin/user" class="btn btn-primary"><i class="mdi mdi-reply-all"></i></a> Chỉnh sửa tài khoản người dùng</h3>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-4">
                                        <input type="text" id="email" name="email" class="form-control" placeholder="Tài khoản người dùng" value="{{ old('email') !== null ? old('email') : $user['email'] }}" required readonly/>
                                    </div>
                                        ...
                                        <select class="select2 m-b-10" id="address_1" name="address[]" style="width: 100%" data-placeholder="Chọn Tỉnh">
                                            ...
                                            @if($address)
                                              @foreach($address as $a)
                                                ...
                                                @elseif(isset($user['address']) && $user['address'][0])
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="form-group row">
                                    <label class="control-label col-md-2 text-right p-t-10">Địa chỉ</label>
                                    <div class="col-md-10">
                                        <input type="text" id="address_4" name="address[]" class="form-control" placeholder="Số nhà, tên đường, khóm, ấp,..." value="{{ isset($user['address'][3]) ? $user['address'][3] : '' }}">
                                    </div>
                                </div>
                            </div>
                        </div>
...
@section('js')
  <script src="{{ env('APP_URL') }}assets/admin/libs/select2/select2.min.js" type="text/javascript"></script>
  <script src="{{ env('APP_URL') }}assets/admin/libs/switchery/switchery.min.js"></script>
  <script type="text/javascript" src="{{ env('APP_URL') }}assets/admin/libs/isotope/isotope.pkgd.min.js"></script>
  <script type="text/javascript" src="{{ env('APP_URL') }}assets/admin/libs/magnific-popup/jquery.magnific-popup.min.js"></script>
  <script src="{{ env('APP_URL') }}assets/admin/js/drag-arrange.min.js"></script>
  <script src="{{ env('APP_URL') }}assets/admin/js/script.js" type="text/javascript"></script>
  <script type="text/javascript">
       $(document).ready(function() {
          ...
          $("#store").keyup(function(){
            var _this = $(this);
            $.get('{{ env('APP_URL') }}slug?str='+ $("#store").val(), function(data){
              $("#slug").val(data);
            });
          });
      });
