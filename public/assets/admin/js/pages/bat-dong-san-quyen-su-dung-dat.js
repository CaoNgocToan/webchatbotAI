...
function ThemDongSuDung(app_url){
    $("#them_dongsudung").click(function(){
        var html = $("#dongsudung_html").html();
        html = html.replace(/select_dongsudung/g,"select2");
        $("#dongsudung_list").append(html);
        $(".select2").select2();
        XoaDongSuDung();
        get_so_giay_chung_nhan(app_url);
    });
}
function XoaDongSuDung(){
    $(".xoa_dongsudung").click(function(){
        var _this = $(this);
        _this.parents(".items").remove();
    });
}
...
function get_so_giay_chung_nhan(app_url){
    $('.so_giay_chung_nhan').keydown(function(e) {
        var code = e.keyCode || e.which;
        var sgcn = $(this).val();
        var index = 0;
        if (code === 9) {
            e.preventDefault();
            $('input[name^="so_giay_chung_nhan"]').each(function() {
                if($(this).val()==sgcn){
                    index++;
                }
            });
            if(index>1){
                alert('Số đã bị trùng');
                return 0;
            }
            var path = app_url + sgcn;
